...
if tmux has-session -t "$SESSION_NAME" >/dev/null 2>&1; then
    ...
    tmux send-keys -t 2 "source /opt/ros/humble/setup.bash; colcon build --packages-select lx_operation && \
        source /home/lx_autonomy/lx_autonomy_ws/install/setup.bash && ros2 launch lx_operation operation.launch.py" C-m
    
    # tmux send-keys -t 2 "source /opt/ros/humble/setup.bash; \
    #     source /home/lx_autonomy/lx_autonomy_ws/install/setup.bash && ros2 launch " C-m
    
    tmux send-keys -t 3 "ros2 action send_goal /operations/autodig_action lx_msgs/action/AutoDig "{}" " C-m
