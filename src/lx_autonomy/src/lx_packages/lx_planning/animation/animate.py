...
def getFrame(i, berm_pts_arr, excavation_pts_arr, height_grid_arr, corners_arr, corners_tool_arr, robot_pose_arr, path_arr, task_arr):
    ...
    # join robot and tool
    artists.append(plt.plot([robot_pose_arr[i][0], robot_pose_arr[i][2]], [robot_pose_arr[i][1], robot_pose_arr[i][3]], '0.5'))
    # visualize the robot as rectangle
    artists.append(plt.gca().add_patch(patches.Polygon(corners_arr[i], color='0.5')))
    # visualize the tool
    artists.append(plt.gca().add_patch(patches.Polygon(corners_tool_arr[i], color='0.5')))
...
def get_berm(theta: float, length: int, height: int):
    ...
    # plt.imshow(berm_grid.T, cmap='Greys', origin='lower')
    # plt.colorbar(label='Value')
    # plt.xlabel('X')
    # plt.ylabel('Y')
...
if __name__ == '__main__':
    ...
    # add constant height of 2 cm
    height_grid += 2
    height_grid[0, 0] = 0
    height_grid[0, 1] = desired_berm_height

    # set figure size
    fig.set_size_inches(height_grid.shape[0]/100, height_grid.shape[1]/100)
    ...
    for out in output_points:
        ...
        if task == 0: # navigation
            ...
            for i in range(0, exc_time):
                height_grid_arr.append(height_grid.copy())
                corners_arr.append(robot.get_corners())
                corners_tool_arr.append(robot.get_corners_tool())
                robot_pose_arr.append([robot.x, robot.y, robot.tool_x, robot.tool_y])
                path_arr.append(np.array([]))
                berm_pts_arr.append(berm_points.copy())
                excavation_arr.append(excavation_points.copy())
                task_arr.append(task_types[task])
