...
void BermEvaluation::bermEval(const nav_msgs::msg::OccupancyGrid::SharedPtr msg){
    ...
    for(long unsigned int i = 0; i < berm_points.size(); i++){
        for(long unsigned int j = 0; j < msg->data.size(); j++){
    ...
    // print the heights
    for(long unsigned int i = 0; i < berm_heights.size(); i++){
        RCLCPP_INFO(this->get_logger(), "Percentage completion at point %ld: %d", i, berm_heights[i]);
    }
