...
class HardwareMux: public rclcpp::Node
{
    private:
        ...
        double drum_current_scale = -0.0314; // Scale for drum current analogRead to Amps
        double acc_current_scale = -0.0314; // Scale for linear actuator current analogRead to Amps
        double drum_current_offset = 24.6; // Offset for drum calibration
        double acc_current_offset = 24.6; // Offset for linear actuator calibration
